...
class Traffic:
    ...
    def step(self, **kwargs):
        ...
        if n_agent_diff > 0:
            self.add_agents(n_agent_diff + 2)
    ...
    def add_agents(self, num=1, init=False):
        if hasattr(self.velocity_distr, 'rvs'):
            vels = self.velocity_distr.rvs(num)
        else:
            vels = np.ones(num) * self.velocity_distr

        if hasattr(self.track_distr, 'rvs'):
            tracks = self.track_distr.rvs(num)
        else:
        ...
        if hasattr(self.vert_rate_distr, 'rvs'):
            vert_rates = self.vert_rate_distr.rvs(num)
        else:
            vert_rates = np.ones(num) * self.vert_rate_distr
        new_velocities = np.hstack((xy_vels, vert_rates[:, None]))

        new_positions = np.vstack((
            ss.uniform(*self.x_bounds).rvs(num),
            ss.uniform(*self.y_bounds).rvs(num),
            ss.uniform(*self.z_bounds).rvs(num),
        )).T
...
class Simulation:
    ...
    def run(self):
        ...
        for t in range(self.steps):
            ...
            self.conflict_log[t] = n_conflicts
    ...
    def setup_data_logging(self):
        self.conflict_log = np.zeros(self.steps)
...
if __name__ == '__main__':
    ...
    sim = Simulation(tfc, own, conflict_dists=(15, 10))
    sim.run()
    sim.end()
    sim.conflict_log.sum()
